...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!categoryIdCheck.valid || !titleCheck.valid || !descriptionCheck.valid) {
        ...
        const categoryIndex = categories.findIndex((c: { id: string }) => c.id === categoryIdCheck.value)
...
export async function PUT(request: NextRequest) {
    ...
    try {
        ...
        if (body.updateCategory && body.categoryId) {
            const catIdx = categories.findIndex((c: { id: string }) => c.id === body.categoryId)
            if (catIdx === -1) {
                return NextResponse.json({ error: 'Category not found' }, { status: 404 })
            }
        ...
        // Update service
            ...
            for (const cat of categories) {
                const svcIdx = cat.services?.findIndex((s: { id: number }) => s.id === body.serviceId)
            ...
            return NextResponse.json({ error: 'Specify serviceId or categoryId with updateCategory' }, { status: 400 })
        }
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        for (const cat of categories) {
            ...
            cat.services = cat.services?.filter((s: { id: number }) => s.id !== parseInt(serviceId))
