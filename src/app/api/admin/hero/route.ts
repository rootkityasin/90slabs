  ...
  try {
    ...
    if (body.primaryCta) {
      updateData.primaryCta = {
        text: body.primaryCta.text
          ? validateInput(body.primaryCta.text, "string", 50).value
          : heroDoc.primaryCta?.text,
        href: body.primaryCta.href
          ? validateInput(body.primaryCta.href, "string", 100).value
          : heroDoc.primaryCta?.href,
      };
    }
    if (body.secondaryCta) {
      updateData.secondaryCta = {
        text: body.secondaryCta.text
          ? validateInput(body.secondaryCta.text, "string", 50).value
          : heroDoc.secondaryCta?.text,
        href: body.secondaryCta.href
          ? validateInput(body.secondaryCta.href, "string", 100).value
          : heroDoc.secondaryCta?.href,
      };
    }

    await db
      .collection("hero")
      .updateOne({ _id: heroDoc._id }, { $set: updateData });
